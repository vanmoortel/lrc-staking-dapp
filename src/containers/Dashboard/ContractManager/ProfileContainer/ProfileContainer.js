...
const ProfileContainer = () => {
  ...
  const getTotalStake = useCallback(() => dispatch(stakingGetTotalStake(stakingContract)), [dispatch, stakingContract]);
  const getStakeList = useCallback(() => dispatch(stakingGetStakeList(stakingContract, library)), [dispatch, stakingContract, library]);
  const getYourStake = useCallback(() => dispatch(stakingGetYourStake(stakingContract, walletAddress.value)), [dispatch, stakingContract, walletAddress]);
  const getFeeStats = useCallback(() => dispatch(feeGetFeeStats(feeContract)), [dispatch, feeContract]);
  const logout = useCallback((_walletID) => dispatch(settingsLogout(deactivate, _walletID)), [dispatch, deactivate]);
  ...
  useEffect(() => {
    computeYourShareAndYourTokenAge(stakeList, yourStake, yourShare, yourTokenAge, setYourShare, setYourTokenAge);
  ...
  return (
    <Profile messages={messages} feesTotal={(feeStats.value.remainingReward || 0) / (10**18)}
                  stakeTotal={totalStake.value.balance / (10**18)} stake={(yourStake.value.balance || 0) / (10**18)}
                  pendingReward={(yourStake.value.pendingReward) / (10**18)} share={yourShare || 0}
                  tokenAge={yourTokenAge || 0} walletAddress={walletAddress.value} tokenAgeList={tokenAgeList||[]}
                  onEditAddress={() => logout(1)} onLogout={() => logout(0)}
